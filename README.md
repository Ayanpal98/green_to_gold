...
The Workflow:
User selects a crop and district.
Input parameters like Clump Age, Species Variety, and Planting Density.
The AI Engine calculates optimal harvest volumes and suggests a "Harvest or Hold" status.
Decision history is persisted and accessible for audit.
2. SHG Activity Ledger (Real-time Tracker)
Designed for Self-Help Groups (SHGs) and Cooperatives.

The Workflow:
SHGs log harvest activities (Cooperative name, District, Volume).
The system uses Optimistic UI updates to show changes instantly.
Table views include CRUD (Create, Read, Update, Delete) capabilities with real-time metric recalculations for total volume and income.
